...
## 7. Version Tagging & Testing
- **Version Tagging**: Whenever we complete a feature, we must use `git tag` to tag the completion (version tagging). Start tags from `2.5.0.1`. Major version number increments require explicit instruction from the user.
- **Comprehensive Testing**: Every feature must have both Feature tests and Unit tests. Every agent must obey this rule.
