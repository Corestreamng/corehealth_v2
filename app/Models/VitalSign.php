...
class VitalSign extends Model implements Auditable
{
    ...
    public function getVitalStatus(string $vital): string
    {
        ...
        switch ($vital) {
            ...
            case 'blood_pressure':
                ...
                if ($sysStatus === 'vital-critical' || $diaStatus === 'vital-critical') {
                    return 'vital-critical';
                }
                if ($sysStatus === 'vital-warning' || $diaStatus === 'vital-warning') {
    ...
    {
        if ($range->critical_min !== null && $val < $range->critical_min) {
            return 'vital-critical';
        }
        if ($range->critical_max !== null && $val > $range->critical_max) {
            return 'vital-critical';
        }

        if ($range->warning_min !== null && $val < $range->warning_min) {
            return 'vital-warning';
        }
        if ($range->warning_max !== null && $val > $range->warning_max) {
            return 'vital-warning';
        }
