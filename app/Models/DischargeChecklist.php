...
/**
 * DischargeChecklist Model
 *
 * Tracks completion of discharge checklist for a specific admission.
 *
 * Workflow:
 * 1. Doctor requests discharge via encounter
 * 2. System creates DischargeChecklist from default template
 * 3. Nurse completes checklist items (billing, instructions, etc.)
 * 4. Once complete, nurse can finalize discharge
 * 5. Bed is released
 *
 * Status flow:
 ...
 * @see App\Models\DischargeChecklistItem
 * @see App\Models\ChecklistTemplate
 */
class DischargeChecklist extends Model implements Auditable
{
    use HasFactory;
    ...
    {
        return $this->hasMany(DischargeChecklistItem::class, 'discharge_checklist_id');
    }
    ...
    public static function createFromTemplate(
        ...
        $template = $template ?? ChecklistTemplate::getDefaultDischarge();
        ...
        if ($template) {
            foreach ($template->items as $templateItem) {
                DischargeChecklistItem::create([
                    'discharge_checklist_id' => $checklist->id,
    ...
    public function updateStatus(int $userId): void
    {
        ...
        if ($completedCount === 0) {
            ...
            $this->admissionRequest->update([
                'admission_status' => 'discharge_checklist',
            ]);
        } else {
