...
/// Chat thread screen for the patient app.
class PatientChatThreadScreen extends StatefulWidget {
  final PatientApiService api;
  final Map<String, dynamic> conversation;

  const PatientChatThreadScreen({
    super.key,
    required this.api,
  ...
  @override
  State<PatientChatThreadScreen> createState() =>
      _PatientChatThreadScreenState();
}

class _PatientChatThreadScreenState extends State<PatientChatThreadScreen> {
  ...
  int get _convId {
    final v = widget.conversation['id'];
  ...
  void _loadMyUserId() {
    ...
    if (json != null) {
      final patient = jsonDecode(json);
      _myUserId = patient['user_id'] ?? patient['id'];
    }
  }
  ...
  Future<void> _loadMessages() async {
    ...
    final res = await widget.api.getMessages(_convId);
    if (!mounted) return;
    if (res.success && res.data is List) {
      final msgs = (res.data as List).whereType<Map<String, dynamic>>().toList();
      setState(() {
        _messages = msgs;
        _loading = false;
  ...
  Future<void> _poll() async {
    if (_lastMessageId == null || _polling) return;
    _polling = true;
    try {
      final res =
          await widget.api.getMessages(_convId, afterId: _lastMessageId);
      if (!mounted) return;
      if (res.success && res.data is List) {
        final newMsgs = (res.data as List).whereType<Map<String, dynamic>>().toList();
  ...
  void _markAsRead() {
    widget.api.markChatAsRead(_convId);
  }
  ...
  Future<void> _send() async {
    ...
    _msgCtrl.clear();
    final res = await widget.api.sendChatMessage(_convId, text);
    if (!mounted) return;
    setState(() => _sending = false);
    if (res.success) {
      final msg = res.data is Map<String, dynamic> ? res.data : null;
  ...
  void _deleteMessage(int msgId, int index) async {
    ...
    if (confirmed == true) {
      final res = await widget.api.deleteChatMessage(msgId);
        ...
        child: Row(
          ...
          children: [
            Icon(Icons.attach_file,
                size: 16, color: isMine ? cs.onPrimary : cs.primary),
            const SizedBox(width: 6),
            Flexible(
              child: Text(
                name,
