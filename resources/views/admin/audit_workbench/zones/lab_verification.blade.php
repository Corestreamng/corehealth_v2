...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h4 class="mb-0 text-primary"><i class="mdi mdi-microscope"></i> Lab Price Verification</h4>
    <div>
                        ...
                        <tr class="{{ $request->is_queried && is_null($request->query_resolved_at) ? 'table-warning' : '' }}">
                            ...
                            <td>
                                <div class="d-flex align-items-center">
                                    <div class="avatar avatar-sm me-2 bg-info-subtle text-info rounded-circle d-flex align-items-center justify-content-center" style="width:32px; height:32px;">
                            ...
                            <td class="text-end pe-4">
                                <div class="d-flex justify-content-end gap-1">
                                    ...
                                    @elseif(isset($request->is_queried) && $request->is_queried && is_null($request->query_resolved_at))
                                        <button class="btn btn-sm btn-warning text-dark font-weight-bold" onclick="openResolveQueryModal('LabServiceRequest', {{ $request->id }})" title="Queried: {{ $request->query_notes }}">
                                            <i class="mdi mdi-alert-circle"></i> Resolve Query
                                        </button>
                                    @else
                                        <button class="btn btn-sm btn-outline-warning" onclick="openRaiseQueryModal('LabServiceRequest', {{ $request->id }})" title="Raise Query">
                                            <i class="mdi mdi-help-circle-outline"></i>
                                        </button>
                                        <button class="btn btn-sm btn-outline-success audit-tick-btn" onclick="markAudited('LabServiceRequest', {{ $request->id }}, this)">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="text-center py-5 text-muted">
                                <i class="mdi mdi-microscope mdi-48px text-light mb-2"></i>
                                <h5>No Lab Requests found</h5>
                                <p>There are no lab requests available for auditing at this time.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
