{{--
    ...
    Config: enable_structured_dose (DB boolean), default_dose_mode ('simple'|'structured')
...
@php
    $prefix = $prefix ?? '';
    $enableStructured = (bool) (appsettings('enable_structured_dose') ?? 1);
    $defaultMode      = appsettings('default_dose_mode') ?? 'structured';
    // If structured is globally disabled, force default to simple
    if (!$enableStructured) { $defaultMode = 'simple'; }
@endphp
...
<div class="dose-mode-toggle-group mb-3">
    <div class="btn-group btn-group-sm" role="group" aria-label="Dose entry mode">
        <input type="radio" class="btn-check" name="{{ $prefix }}dose_mode"
               ...
               {{ $defaultMode === 'simple' ? 'checked' : '' }}>
        <label class="btn btn-outline-secondary" for="{{ $prefix }}dose_mode_simple">
            <i class="fa fa-pen"></i> Simple Note
        </label>

        <input type="radio" class="btn-check" name="{{ $prefix }}dose_mode"
               id="{{ $prefix }}dose_mode_structured" value="structured" autocomplete="off"
               {{ $defaultMode === 'structured' ? 'checked' : '' }}>
...
@else
{{-- Structured dose globally disabled — silently force simple mode --}}
<input type="hidden" name="{{ $prefix }}dose_mode" value="simple">
@endif
