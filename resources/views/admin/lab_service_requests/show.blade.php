@php
    ...
    $labNumber = $req->lab_number ?? str_pad($req->id, 7, '0', STR_PAD_LEFT);
    $resultByName = $req->resultBy ? userfullname($req->resultBy->id) : null;
    $approverName = ($req->approver) ? userfullname($req->approver->id) : null;
    $sampleDate = $req->sample_date ? \Carbon\Carbon::parse($req->sample_date)->format('d M Y, h:i A') : null;
    ...
    $serviceName = $req->service->service_name ?? 'Laboratory Test';
...
<head>
    <meta charset="UTF-8">
    <title>Lab Report - {{ $serviceName }} - {{ $patientName }}</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .rpt-header .report-label .lab-no { font-size: 10.5px; color: #444; margin-top: 4px; font-weight: 600; }
        ...
        .pt-field.full-width { flex-basis: 100%; }
    ...
    <div class="page">
        ...
        <div class="rpt-header">
            <div class="rpt-header-top">
                ...
                <div class="report-label">
                    <div class="tag">Laboratory Report</div>
                    <div class="lab-no">Lab # {{ $labNumber }}</div>
        ...
        <div class="pt-banner">
            ...
            <div class="pt-col">
                ...
                @endif
                @if($req->patient && $req->patient->genotype)
                    <div class="pt-field"><span class="lbl">Genotype:</span> <span class="val">{{ $req->patient->genotype }}</span></div>
                @endif
            </div>
        </div>
        ...
        <div class="meta-row">
            ...
            @if($sampleDate)
                <div class="meta-item"><span class="lbl">Sample Date:</span> <span class="val">{{ $sampleDate }}</span></div>
            @endif
            @if($resultDate)
            ...
            @if($resultByName)
                <div class="sig-box">
                    <div class="sig-line">
                        ...
                        <div class="sig-role">Medical Laboratory Scientist</div>
                    </div>
                </div>
            @endif
            @if($approverName)
        ...
        <div class="rpt-footer">
            <p>This is a computer-generated laboratory report. Results should be interpreted in the context of clinical findings.</p>
