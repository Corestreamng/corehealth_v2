...
<style>
    ...
    .cmd-bar-title .proc-name {
        font-size: 1.45rem;
        font-weight: 700;
        margin: 0;
    }

    .cmd-bar-title .proc-meta {
        font-size: .82rem;
        opacity: .9;
        display: flex;
        flex-wrap: wrap;
    ...
    .cmd-bar-title .proc-meta span i {
        margin-right: 3px;
    }
    ...
    .consent-strip.cs-warn {
        background: #fff8f0;
        color: #7d4e0f;
    }

    .consent-strip.cs-danger {
        background: #fff5f5;
        color: #721c24;
        border-left: 4px solid #dc3545;
    }

    .consent-strip.cs-grey {
        background: #f8f9fa;
        color: #495057;
    }

    .consent-strip .cs-detail {
        font-size: .78rem;
        opacity: .8;
        margin-left: 4px;
    }

    .consent-strip .cs-action {
        margin-left: auto;
    }
    ...
    .patient-cmd-meta span {
        margin-right: 10px;
    }

    .patient-cmd-bar .ml-auto {
        margin-left: auto;
    }
    ...
    .note-card-body .note-content-preview {
        max-height: 80px;
        overflow: hidden;
        transition: max-height .3s;
    }

    .note-card-body .note-content-preview.expanded {
        max-height: 9999px;
    }

    .note-card-body .note-toggle {
        font-size: .75rem;
        color: var(--proc-primary);
        cursor: pointer;
    }
    ...
    .consent-big-card.cs-ok {
        border-color: #28a745;
        background: #f0fff4;
    }

    .consent-big-card.cs-warn {
        border-color: #fd7e14;
        background: #fff8f0;
    }

    .consent-big-card.cs-danger {
        border-color: #dc3545;
        background: #fff5f5;
    }

    .consent-big-card.cs-grey {
        border-color: #adb5bd;
        background: #f8f9fa;
    }
    ...
    .consent-option-btn.selected-obtained {
        border-color: #28a745;
        background: #f0fff4;
    }

    .consent-option-btn.selected-waived {
        border-color: #ffc107;
        background: #fffdf0;
    }

    .consent-option-btn.selected-not_required {
        border-color: #6c757d;
        background: #f8f9fa;
    }

    .consent-option-btn.selected-pending {
        border-color: #fd7e14;
        background: #fff8f0;
    }

    .consent-option-btn .opt-label {
        font-weight: 600;
        font-size: .85rem;
        line-height: 1.2;
        display: block;
    ...
    .notes-tabs .nav-link.active {
        color: var(--proc-primary);
        border-bottom-color: var(--proc-primary);
    ...
    .chosen-rich-preview .cr-title {
        font-size: .86rem;
        font-weight: 700;
        color: #2c3e50;
    }

    .chosen-rich-preview .cr-meta {
        margin-top: 5px;
        display: flex;
        flex-wrap: wrap;
        gap: 8px;
    ...
    .chosen-rich-preview .cr-pay {
        color: #d97a00;
        font-weight: 700;
        font-size: .78rem;
    }

    .chosen-rich-preview .cr-claim {
        color: #1b8f4f;
        font-weight: 700;
        font-size: .78rem;
    }

    .chosen-rich-preview .cr-base {
        color: #6c757d;
        font-size: .75rem;
    }

    .chosen-rich-preview .cr-mode {
        font-size: .68rem;
        font-weight: 700;
        padding: 2px 6px;
        border-radius: 4px;
    ...
    .chosen-rich-preview .cr-mode.mode-express {
        background: #d4edda;
        color: #155724;
    }

    .chosen-rich-preview .cr-mode.mode-primary {
        background: #cce5ff;
        color: #004085;
    }

    .chosen-rich-preview .cr-mode.mode-secondary {
        background: #fff3cd;
        color: #856404;
    }

    .chosen-rich-preview .cr-mode.mode-hmo {
        background: #d1ecf1;
        color: #0c5460;
    }

    .chosen-rich-preview .cr-mode.mode-cash {
        background: #f1f3f5;
        color: #495057;
    }
    ...
    .billing-search-hmo-mode.mode-hmo {
        background: #d1ecf1;
        color: #0c5460;
    }

    .billing-search-hmo-mode.mode-cash {
        background: #f1f3f5;
        color: #495057;
    }
    ...
    /* Role-based ordering */
    [data-page-role="nurse"] .clinical-tab-flex {
        display: flex;
        flex-direction: column;
    }
    ...
    [data-page-role="admin"] .clinical-tab-flex {
        display: flex;
        flex-direction: column;
    }

    [data-page-role="doctor"] .clinical-or-panel,
    [data-page-role="admin"] .clinical-or-panel {
        order: 1;
    }
                                ...
                                <div class="tab-pane fade" id="proc-orders-meds">
                                    <div class="history-table-wrapper p-2">
                                        <table class="table table-borderless" id="procedure_meds_history" style="width:100%">
                                            <thead>
                                                <tr>
                                                    <th><i class="mdi mdi-pill"></i> Medication Requests</th>
                                                </tr>
                                            </thead>
                                        </table>
                                    </div>
            ...
            <div class="section-card">
                ...
                <div class="section-card-body">
                    <div id="outcome-display" style="{{ $procedure->outcome ? '' : 'display:none;' }}">
                        ...
                        @else
                        <div class="empty-state py-2"><i class="fa fa-flag"></i>
                    ...
                    @if(!$procedure->outcome)
                    <div class="empty-state py-2"><i class="fa fa-flag"></i>
                        <p>No outcome recorded<br><small>Only doctors can record outcomes</small></p>
                    </div>
                    @endif
                    @endhasanyrole
                </div>
...
@include('admin.patient-procedures.partials._scripts')
