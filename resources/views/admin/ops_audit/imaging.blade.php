...
@section('title', 'Ops Audit — Imaging')

@section('ops_audit_content')
<div class="d-flex justify-content-between align-items-center mb-3">
    <div>
        <h5 class="font-weight-bold mb-0"><i class="mdi mdi-radiology-box text-primary me-1"></i> Imaging Audit</h5>
        <small class="text-muted">All Imaging Requests, Billed Scans, Cashbook</small>
    </div>
...
{{-- Tabs --}}
<ul class="nav nav-tabs ops-tabs mb-0" id="imagingTabs" role="tablist">
    <li class="nav-item">
        <a class="nav-link active" id="tab-requests" data-bs-toggle="tab" href="#pane-requests" role="tab">
            <i class="mdi mdi-radiology-box me-1"></i> Imaging Requests
        </a>
    </li>
    <li class="nav-item">
        <a class="nav-link" id="tab-bills" data-bs-toggle="tab" href="#pane-bills" role="tab">
            <i class="mdi mdi-receipt me-1"></i> Billed Scans
        </a>
    </li>
    <li class="nav-item">
...
<div class="tab-content bg-white border border-top-0 rounded-bottom p-3">
    ...
    <div class="tab-pane fade show active" id="pane-requests" role="tabpanel">
        <div class="row g-2 mb-3 ops-kpi-row" id="kpi-requests"></div>

        <div class="row g-2 mb-2">
            <div class="col-md-2">
                <select name="status" class="form-select form-select-sm ops-tab-filter" data-tab="requests">
        ...
        <div class="table-responsive">
            <table class="table table-sm table-bordered table-striped ops-datatable w-100" id="dt-requests">
                <thead>
                    <tr>
                        <th>Date</th>
                        <th>Patient</th>
...
$(function() {
    var dataUrls = {
        requests: "{{ route('ops-audit.imaging.data', 'requests') }}",
        bills: "{{ route('ops-audit.imaging.data', 'bills') }}",
        requisitions: "{{ route('ops-audit.imaging.data', 'requisitions') }}",
        cashbook: "{{ route('ops-audit.imaging.data', 'cashbook') }}"
    };
    ...
    $('a[data-bs-toggle="tab"]').on('shown.bs.tab', function(e) {
        ...
        setTimeout(function() {
            $.fn.dataTable.tables({ visible: true, api: true, order: [] }).columns.adjust();
        }, 200);
    });
