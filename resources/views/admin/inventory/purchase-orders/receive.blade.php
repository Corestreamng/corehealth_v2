                ...
                <table class="table table-sm mb-0">
                    ...
                    <tbody>
                        ...
                        <tr class="item-main-row" data-item-id="{{ $item->id }}">
                            ...
                            <td class="text-center align-middle text-warning"><strong class="pending-pieces">{{ $pending }}</strong></td>
...
<script>
    ...
    // Update base-unit equivalent display and max qty on packaging change
    $(document).on('change', '.receive-packaging-select', function() {
        var row = $(this).closest('tr');
        var select = $(this);
        var baseFactor = parseFloat(select.find('option:selected').data('base-qty')) || 1;
        var pendingPieces = parseInt(row.find('.pending-pieces').text()) || 0;

        // Price scaling logic
        var prevFactor = parseFloat(select.data('prev-factor')) || 1;
        var costInput = row.find('.receive-cost');
        var currentCost = parseFloat(costInput.val()) || 0;

        if (currentCost > 0) {
            var newCost = (currentCost / prevFactor) * baseFactor;
            costInput.val(parseFloat(newCost.toFixed(4)));
        }
        select.data('prev-factor', baseFactor);

        var maxPkg = Math.floor(pendingPieces / baseFactor);
        var qtyInput = row.find('.receive-qty');

        qtyInput.attr('max', maxPkg);
        if (parseInt(qtyInput.val()) > maxPkg) {
            qtyInput.val(maxPkg);
        }

        updateReceiveBaseEquiv(row);
    });
    ...
    $(function() {
        $('.item-main-row').each(function() {
            ...
            pkgSelect.data('prev-factor', baseQty);
            
            // Initial scale if pre-selected
            var costInput = row.find('.receive-cost');
    ...
    function receiveItems() {
        ...
        $('.receive-qty').each(function() {
            ...
            if (qty > 0) {
                ...
                const pkgSelect = $(`.receive-packaging-select[data-item-id="${itemId}"]`);
                const baseFactor = parseFloat(pkgSelect.find('option:selected').data('base-qty')) || 1;

                items.push({
                    item_id: itemId,
                    product_id: productId,
                    qty: Math.round(qty * baseFactor),
                    batch_number: batchNumber,
                    expiry_date: expiryDate || null,
                    actual_cost: costPrice ? parseFloat((costPrice / baseFactor).toFixed(4)) : null,
                    selling_price: sellingPrice ? parseFloat((sellingPrice / baseFactor).toFixed(4)) : null,
                    manufacture_date: manufactureDate || null,
                    received_packaging_id: pkgSelect.val() || null,
                    received_packaging_qty: pkgId = pkgSelect.val() ? qty : null
                });
            }
        });
